...
Options:
  ...
  -f, --feature <id>   Feature identifier (required for all roles)
...
async function run() {
  ...
  if (!parsed.feature) {
    throw new Error("--feature is required for all roles");
  }
  const validatedFeatureId = validateFeatureId(parsed.feature);
