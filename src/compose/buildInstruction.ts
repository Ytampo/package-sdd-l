...
function buildRoleNotePolicy(role: Role): string[] {
  ...
  if (role === "coder") {
    return [
      ...
      "- Use `sdd-l-prompts/templates/change-notes-template.md` as a reference when writing markdown files.",
      "- Decide whether to create a new file or update an existing file based on context.",
  ...
  return [
    ...
    "- The template is not for chat-only output.",
    "- Use `sdd-l-prompts/templates/teaching-note-template.md` as a reference when writing markdown files.",
    "- Decide whether to create a new file or update an existing file based on context.",
